...
不根据器件型号或位号猜测。沿真实引脚搜索，在首个可行的连接距离层内比较完整线长
（含命名引线）、线段数、轴线对齐误差及可见包络面积，不再直接接受第一个合法坐标。
局部电源/地优先尝试 80 raw 内的无碰撞直线/L 形合并，再生成 GND、电源、信号标记。
短距离阈值是有界启发式，不证明更远不能直连；仍保留每棵线树必需的命名点。
这不是全局最优或跨器件回溯求解，失败不能用放宽电气/文字检查来兜底。
...
功能 zone 是一个已确认核心芯片/接口与其专属外围的布局所有权边界，不是 Lib 库资产，
