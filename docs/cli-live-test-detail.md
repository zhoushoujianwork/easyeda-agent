...
工程空间按用户选定的验收范围执行并写入总报告。2026-09-25 本轮用户明确暂不测试团队，
因此 B03 验收个人空间的创建/查找/打开/导出；团队创建列为范围外未验证，不借此声明团队可用。
范围内任一实际失败仍不放行，其余 B00–B10 判据保持不变。
...
该次高级准入按[简版中的范围例外](cli-live-test.md#放行条件)执行；不要求先修复三项或仅因此重跑基础全集，
也不将历史 B04/B08 失败、dev.11 未执行项写成通过。单项实际回包与错误退出契约不变。
...
- B05：保存是否发生、是否读网表/引脚/属性、是否同页 reopen 分别控制，禁止把相关时序直接当根因。
- B06：标签调用前冻结完整属性清单；调用一次后即使回包空也做 fresh 差分，核对新增 Name 属性、
  位置、可见性、父导线网名，再 save/reload 和导出。不得因错误回包自动重试。
- B08：`no-pours` 等禁布区不传 `--name`；命名对照使用 `--rule follow-rule`，名称承担 DRC
  规则匹配语义。分别验证几何与规则；无效组合应写前拒绝，不能要求宿主保存不适用字段。
- B10：过期 PCB 器件 ID 必须先与 fresh inventory 核对；回归测试证明 modify 调用次数为 0，
  现场再用前后完整快照证明无副作用。
