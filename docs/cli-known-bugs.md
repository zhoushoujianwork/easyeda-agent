...
- 高级续测新发现 ESP32 模组 `prim-delete` 返回 `partial`，400 ms settle 后仍存在，
  ...
  该能力的高级现场用例，最终精确清理并保存重载回空白基线。根因未知，不认定与 #256/
  ...
  后续隔离确认网表导出单独可触发故障；`1.7.1-dev.1` 已补焦点恢复候选和删除回读三态，
  离线检查通过，现场因工程打开超时尚未复测，不关闭此项。见[修复记录](reviews/2026-09-26-sch-delete-fix.md)。
