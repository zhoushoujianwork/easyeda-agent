...
建议采用**受影响操作的定向提示 + 原有失败退出 + fresh 回读**，暂不整体下线。
cmdKey 和 NC 未生效已会报错；后续提示可补上 bug 编号、影响及回读方法。
**这次只登记和约定策略，尚未新增 CLI 专用 warning，也未修改退出码或禁用开关。**
...
- 允许继续使用现有功能，不代表基础 B00–B10 总门禁通过，也不补签高级 CLI 验收。
