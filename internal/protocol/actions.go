...
func AllActions() []ActionSpec {
	return []ActionSpec{
		...
		// Build polygon (pcb_MathPolygon.createPolygon) → pour.create → rebuild.
		{
			...
			Mutates:     true,
			NeedsWindow: true,
			Description: "Create a net-bound copper pour boundary from points ([[x,y],...] mil, y-up). Fresh exact-ID readback verifies net/layer/fill/geometry and any explicit name/priority/lineWidth before rebuilding copper, then verifies again. Mismatch or unavailable readback retains primitiveId with partial:true/verified:false; CLI/Apply stop without retry. verified describes boundary parameters, while poured/rebuildError separately describe materialization; inspect pcb.poured.list and DRC for actual copper and connectivity.",
			Inputs:      []string{"points ([[x,y],...] mil)", "net required (nonempty)", "layer optional (default 1=TOP)", "fill optional (solid|grid|grid45)", "name optional", "priority optional", "lineWidth optional (mil)"},
			Outputs:     []string{"primitiveId", "net", "layer", "fill", "pourName", "priority", "lineWidth", "locked", "source", "verified", "partial?", "differences?", "error?", "poured", "rebuildAttempted", "rebuildError?"},
			VerifyWith:  []string{"pcb.pour.list", "pcb.poured.list", "pcb.drc.check"},
		},
		{
			Name:        "pcb.pour.list",
			Domain:      DomainPcb,
		...
		// exclusion. NOT net-bound filled copper (that's pcb.pour.create).
		{
			...
			Mutates:     true,
			NeedsWindow: true,
			Description: "Create a keep-out / rule region from points ([[x,y],...] mil, y-up). ruleType accepts no-components(2), no-wires(5), no-fills(6), no-pours(7), no-inner-electrical(8), follow-rule(9); default [2,5,7]. A name requires follow-rule. Fresh exact-ID readback verifies layer/rules/geometry/lock and explicit name/lineWidth; mismatches retain actual state with partial:true/verified:false and CLI failure. This is not net-bound copper; use pcb.pour.create for a power/ground boundary.",
