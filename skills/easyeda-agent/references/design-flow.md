...
- **P7 布线**:`pcb route-short`;个别擦焊盘的手工 `pcb track` 绕行。**换层跳线用 `pcb via-hop`**(stub→via→对层 track→via→stub + 自动键合 fill,封死 track↔via 不导通坑,pro-api-sdk#31);单颗错 via/track 用 `pcb via-delete/track-delete --ids` 精准删,别整网 `rip-up`。**注意 `rip-up` 会连电源缝合过孔一起删**——之后重跑 `power-planes` 补回。📸 录制模式:信号布线完成后抓一张阶段截图。
...
- **P10 门**:`pcb drc`(passed)+ `pcb check`(0 issue)双清零,再 `pcb save`。**两条硬注意**:① **via 桥必须配 fill**——4 层/曾有 PLANE 的板,裸 track↔via 结点不注册导通(pro-api-sdk#31);用 `pcb via-hop` 自动键合,或对既有结点补 ~20×20mil 同网 `pcb fill`(见 pcb.md 连通性键合真值表)。② **DRC 需前台**——后台/被遮挡窗口 DRC 重画布计算永不完成;超时就把 EasyEDA 切前台**单发一次**,绝不循环重试(daemon 已防重入,重复下发直接拒 `ACTION_BUSY`)。逐条修错用 `pcb drc --json` 的 `{rule,net,x,y,objs}` 定位,`objs` 直接喂 `via-delete`/`track-delete`。📸 录制模式:DRC 通过的最终态抓一张阶段截图,作为交付收尾图。
