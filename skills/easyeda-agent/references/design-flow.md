...
| **可视化产物门(仅录制/演示)** | 每阶段有没有**非 stale 的原生截图** | `easyeda pcb snapshot` / `easyeda sch snapshot`,对比相邻两帧 `sha256` | 每阶段留档交付 |

**核心纪律——原生截图 ≠ 数据渲染图**:

1. **优先原生 EasyEDA 截图。** 每个阶段用 `easyeda pcb snapshot`(原理图侧 `sch snapshot`)抓画布。响应里带 `sha256`/`capturedAt`。
2. **逐帧 stale 检测。** 把**上一帧的 `sha256`** 与本帧对比:字节相同即判 **stale**(EasyEDA 不会在 API 改动后自动重绘)。原理图侧可直接用 `easyeda sch snapshot --previous-sha256 <上帧sha>`——connector 会自动重试一次并回报 `stale=true`;PCB 侧目前需**手动对比两帧 `sha256`**(相同即 stale)。stale 时先 `easyeda view fit` 或切前台触发重绘再抓,仍 stale 则明确告警。
...
> 可选辅助命令 `easyeda pcb stage-snapshot --stage "P7 routing"`(一条命令内抓原生图 + 批量落盘数据包 + 发阶段通知)尚未实现,当前请**手动**按上述步骤组合 `pcb snapshot` + 数据命令 + `easyeda notify`。
...
> 🎬 **录制/演示模式下额外一步**:发通知的同时,在下列标了 📸 的阶段抓一张**原生截图**并与上帧 `sha256` 对比(见上文「录制/演示模式」)——stale 就重绘重抓,并同步落盘该阶段数据快照(`pcb list`/`track-list`/`via-list`/`pour-list`/`drc`/`check`/`layout-lint`)。
