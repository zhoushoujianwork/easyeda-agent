...
- **兜底引擎 `--engine official`**:块/spec 都没覆盖的散乱页,可让**官方 `eda.sch_Document.autoLayout()`**(@beta,3.2.148 起可用)对整页粗排。**代价**:① 长操作 ~2min(封装已用 300s 超时,`--apply` 才实跑,`--dry-run` 只报将做什么);② 需目标页**前台**(它对前台文档布局,后台会卡);③ 产出是**连通性放射状**,比模板**散且乱**——只作块未命中时的起点,之后仍要 `sch align`/`distribute` 精修。**优先级铁律:命中块→`block-apply` 模板;有 spec→`--engine template`;都没有才 `--engine official` 兜底。**
