...
- **P7 布线**:`pcb route-short`;个别擦焊盘的手工 `pcb track` 绕行。**换层跳线用 `pcb via-hop`**(stub→via→对层 track→via→stub,track↔via 本身即导通,无需键合 fill——pro-api-sdk#31 是我们的误诊已订正);单颗错 via/track 用 `pcb via-delete/track-delete --ids` 精准删,别整网 `rip-up`。**注意 `rip-up` 会连电源缝合过孔一起删**——之后重跑 `power-planes` 补回。📸 录制模式:信号布线完成后抓一张阶段截图。
