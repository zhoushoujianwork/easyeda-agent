...
- ✅ **DFM 审查 (design-for-manufacture audit)** (task #33, done) — `pcb check`: acute-angle / dangling-end /
  overlapping- & single-layer-via / 2-pin width-mismatch / duplicate-segment, reconstructed Go-side from
  placed copper. See the `pcb check` bullet in **Read / inspect**. Absorbs the official DFM tool's
  geometry checks (`docs/marketplace-coverage.md`, HIGH item).
