...
`sheet.flow` 可选 `z`（规划器默认）或 `compact`；CLI `--flow z|compact` 显式覆盖输入选择。
...
输入 `schemaVersion:1/components/netPolicies/zones`，可选 `attachments/maxCandidates/optimization/spacing`；
