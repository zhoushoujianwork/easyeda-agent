...
export function classifyWireConnectivity(
	...
	if (pinEnds === 1) {
		// One end on a pin, the other floating. Only an auto-named net betrays it as
		// an orphaned stub (flag deleted); a real named net means it's mid-route.
		return isAutoGeneratedNet(net) ? 'orphan-stub' : 'connected';
	}
	return 'dangling';
}
