...
- 新增完整队列负例，防止普通 verify 补签、再次创建铺铜或继续依赖写入。dev.10 保留为未接受候选，不进入现场。

## [1.7.1-dev.10] — 2026-09-26

- 铺铜创建先按精确 ID fresh 核对边界参数，再重建并回读；宽度等字段不符或回读未知保留实际值与已创建 ID，不再把参数回显当成功。
- CLI、Apply 和组合铺铜命令对未验证边界失败退出，停止依赖写入；实际铜材料化状态与边界验证分开报告。
- 本地开发修复候选；不修饰 dev.9 的页改名/区域与铺铜宽度失败，不代表基础或高级验收通过。

## [1.7.1-dev.9] — 2026-09-26

- 修复 daemon 将同工程、文档、标签的活跃连接误判为重复并互相关闭，导致连接器持续重连。
- 多个候选保持明确歧义，必须指定窗口；不再按连接先后或版本选窗。退休窗口仅保留诊断，不自动把旧 ID 请求转发到另一连接。
- 增加双 WebSocket 交替上下文/心跳、明确选窗、旧 ID 拒绝、在途写入断线不重放的回归；同步 Skill 的连接身份规则。
- 本地开发修复候选，不代表 B00–B10 或高级 CLI 已验收通过。

## [1.7.1-dev.8] — 2026-09-26

- 页改名不再把宿主 false、异常或未知返回当作成功；一次官方调用后保留有界 fresh 页名证据，只有宿主 true 且读回匹配才验证通过。
- 页名读取失败/缺失/重复/超时不归因为缓存；去除触发无关写操作的建议，迟到读取不会继续调用。
- CLI 兼容读取旧回包时同样拒绝内部 ok=false、verified=false/缺失及 partial，保留原始输出并返回非零。
- 本包修复失败传播；宿主拒绝条件与实际改名能力另行现场核实，不补签 dev.7 B04。

## [1.7.1-dev.7] — 2026-09-26

- 真实 Web V4 沙箱屏蔽裸 Worker/Blob/URL 时，从浏览器 globalThis 创建固定后台时钟；部分创建失败与异步 worker 错误明确降级并清理资源。
- 停用或替换控制器时终止时钟并移除唤醒监听；显式停止连接仍保留进行中动作的期限扫描。
- 延续 dev.5 注册/握手截止与上下文顺序修复、dev.6 daemon 窗口身份保护；这些旧候选均未安装，本包须重新现场验收。

## [1.7.1-dev.6] — 2026-09-26

- 在 dev.5 连接器重连候选基础上，daemon 将 context 绑定到已注册的精确窗口身份，拒绝未注册或身份错配的上下文，不触发重复连接清理。
- 开发候选；需以最终包重跑 B00–B10 和高级验收，不能从 dev.4 的定向删除回归推断全部通过。

## [1.7.1-dev.5] — 2026-09-26

- 重连注册释放等待和握手超时增加 worker 时钟兜底；取消、超时、旧 session 和重复握手不再恢复或替换当前注册。
- 异步 context 绑定原连接身份，并拒绝覆盖较新回读，防止旧请求把新连接登记成旧窗口。
- 后台定时器冻结及跨连接漂移有离线反例；dev.4 B02 的现场根因与新包真实恢复仍待验证，不继承旧版基础通过。

## [1.7.1-dev.4] — 2026-09-26

- Web reload 冻结刷新前全部窗口，拒绝把同工程既有窗口当成新的连接器注册；新窗口中优先验证目标文档。
- 原理图事务就绪与删除实现沿用 dev.3，通过同版基础门禁后再续跑高级验收。

## [1.7.1-dev.3] — 2026-09-26

- 收窄就绪检查改动到原理图调用面；删除事务候选的最终本地验证包。

## [1.7.1-dev.2] — 2026-09-26

- 网表导出及逐批删除增加只读宿主事务就绪检查，等待 RealTimeSync 结束后才调用官方 API；有界超时、身份漂移和未知状态停止后续写入。
- 移除现场无效的导出后焦点恢复候选；保留删除三态回读和最终 CLI 结果修复。
- 本地候选，尚待原始故障现场回归，不是正式发布。

## [1.7.1-dev.1] — 2026-09-26

- 网表导出后通过官方 API 恢复原编辑标签的输入焦点，前后身份不一致或恢复失败时停止调用。
- 原理图删除、级联清理和断连回读采用已删除／存活／未知三态；回读失败和页面漂移不再虚报删除或继续清理连线。
- `sch prim-delete` 输出最终有界复核结果并保留 attempts；仅确认删除的 ID 才移除组登记，未知状态不自动重试。
- 本地修复候选，不是正式发布；高级 CLI 验收仍须独立完成。
