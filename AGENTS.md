...
2026-09-27 用户批准进入 `v1.8.0` 发布流程，并明确移除未解决的页面改名 #55、
区域/铺铜边界线宽 #260、铺铜优先级 #261 对本次高级测试与发布的整体阻断。
这三项作为已接受的已知限制继续跟踪；可据已有基础证据、定向复测和当前目标/基线核对
进入完整用例，不要求先修复三项或为此重跑整套基础检查。历史失败与未运行状态保留，
不称严格 B00–B10 全通过。仅改变本次验收准入，不删除请求/回读校验、错误退出、
